...
mod tests {
    ...
    #[test]
    fn get_state_returns_empty_json_for_empty_engine() {
        let engine_ptr = new_engine(0);
        let json_ptr = get_state(engine_ptr);
        
        assert_ne!(json_ptr, 0);
        
        unsafe {
            let json_str = std::ffi::CStr::from_ptr(json_ptr as *const i8);
            let json_string = json_str.to_str().unwrap();
            let state: ClientFeatures = serde_json::from_str(json_string).unwrap();
            
            assert!(state.features.is_empty());
            assert_eq!(state.version, 2);
        }
        
        free_engine(engine_ptr as *const u32);
    }
    ...
    fn get_state_returns_loaded_features() {
        ...
        let json_str = serde_json::to_string(&client_features).unwrap();
        let json_bytes = json_str.as_bytes();
        
        take_state(engine_ptr, json_bytes.as_ptr() as u32, json_bytes.len() as u32);
        
        let state_ptr = get_state(engine_ptr);
        assert_ne!(state_ptr, 0);
        
        unsafe {
            let json_str = std::ffi::CStr::from_ptr(state_ptr as *const i8);
            let json_string = json_str.to_str().unwrap();
            let state: ClientFeatures = serde_json::from_str(json_string).unwrap();
