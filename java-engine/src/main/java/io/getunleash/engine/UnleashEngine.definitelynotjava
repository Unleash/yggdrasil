...
public class UnleashEngine {
  ...
  UnleashEngine(UnleashFFI ffi, List<IStrategy> customStrategies, IStrategy fallbackStrategy) {
    ...
    if (customStrategies != null && !customStrategies.isEmpty()) {
      List<String> builtInStrategies =
          readRaw(yggdrasil.builtInStrategies(), new TypeReference<List<String>>() {});
  ...
  public void takeState(String toggles) throws YggdrasilInvalidInputException, YggdrasilError {
    read(
        yggdrasil.takeState(this.enginePtr, toUtf8Pointer(toggles)),
  ...
  public void countVariant(String flagName, String variantName) throws YggdrasilError {
    read(
        yggdrasil.countVariant(this.enginePtr, toUtf8Pointer(flagName), toUtf8Pointer(variantName)),
        new TypeReference<YggResponse<Void>>() {});
  }

  public MetricsBucket getMetrics() throws YggdrasilError {
    MetricsBucket response =
        read(
            yggdrasil.getMetrics(this.enginePtr),
  ...
  /** Handle reading from a pointer into an YggdrasilResponse and unwrapping that to an object */
